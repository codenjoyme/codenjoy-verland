...
public class SmokeTest {
    ...
    @Test
    public void test() {
        // about 5 sec -> 7 sec -> 3.5 sec (100.000 / 50 sec) -> (100.000 / 44 sec)
        //   -> (100.000 / 38 sec) -> 2.8 sec (100.000 / 32 sec) -> 2.2 sec (100.000 / 25 sec)
        //  -> 0.86 sec (100.000 / 23 sec)
        int ticks = 1000;
    ...
    @Test
    public void test2() {
        // about 50 sec -> 23 sec -> 3 sec -> 2 sec -> 1.6 sec -> 1.3 sec > 2.4 sec > 0.7 src
        int ticks = 1000;

        SmokeUtils.recheck = actual -> {
            // мы все же проиграли
            assertEquals(true, actual.contains(Event.GOT_INFECTED.name()));
            assertEquals(true, actual.contains(Event.FORGOT_POTION.name()));
            assertEquals(true, actual.contains(Event.SUICIDE.name()));
        };
